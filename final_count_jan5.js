...
async function countForDate(nodeId, date) {
    ...
    const body = {
        ...
        page: 0,
        pageSize: 1000 // High pageSize to get all if it's an array response
    };
...
async function run() {
    try {
        ...
        console.log(`Contando comprobantes para ${date}...`);
        const bio = await countForDate(NODOS.BIOSALUD, date);
        const chacras = await countForDate(NODOS.CHACRAS, date);
        console.log(`\nBIOSALUD: ${bio}`);
        console.log(`CHACRAS: ${chacras}`);
