...
async function debugInsurance(nodeId) {
    ...
    const data = await res.json();
    console.log(`\n--- INSURANCE DATA FOR NODE ${nodeId} ---`);
...
async function run() {
    try {
        await debugInsurance(NODOS.BIOSALUD);
        await debugInsurance(NODOS.CHACRAS);
    } catch (e) {
