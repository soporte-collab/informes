...
async function inspectInvoice() {
    try {
        ...
        const authRes = await axios.post(ZETTI_CONFIG.auth_url, authParams.toString(), {
            headers: { 'Authorization': `Basic ${credsBase64}`, 'Content-Type': 'application/x-www-form-urlencoded' }
        });
        ...
        console.log("creationUser:", data.creationUser);
        console.log("modificationUser:", data.modificationUser);
        console.log("seller:", data.seller);
        console.log("vendedor:", data.vendedor);
        console.log("terminal:", data.terminal);

        if (data.agreements && data.agreements.length > 0) {
            console.log("First Agreement creationUser:", data.agreements[0].creationUser);
        }
