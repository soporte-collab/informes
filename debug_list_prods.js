...
async function listProducts(nodeId) {
    const token = await getZettiToken();
    const url = `${ZETTI_CONFIG.api_url}/v2/${nodeId}/products?pageSize=20`;
    ...
    console.log(`Listed ${content.length} products.`);
}

async function run() {
    try {
        await listProducts('2378041');
        await listProducts('2406943');
    } catch (e) {
